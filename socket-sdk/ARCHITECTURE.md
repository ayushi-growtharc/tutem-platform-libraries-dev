...
## 7. What belongs in the SDK vs your service

The dividing line: the SDK carries generic socket infrastructure and **zero** business knowledge.
...
- The auth hook *interface* — never an implementation
- `MessageDispatcher` (send to user / room / broadcast)
- Serialization
- Heartbeat and ghost-session cleanup
- Metrics and tracing
- The error-handler interface and its default
- Spring lifecycle events

**Your service's side — business logic only:**
...
└── src/main/java/
    ...
    │
    └── service/
          RideService.java          injects MessageDispatcher to push to clients
```

A consuming service should need no `com.corundumstudio.socketio` imports of its own — only the
SDK's annotations and the `MessageDispatcher` / `ConnectionManager` entry points. The exception is
handler method parameters, which may legitimately take `SocketIOClient` or `AckRequest`; that is
why both are `api` dependencies rather than `implementation` (see `build.gradle.kts`).
